...
class FW2DReader(Struct):
    ...
    @classmethod
    def get_doppler_spectra(cls,  subdir, simname = None, machine = 'irene', **kwargs):
        ...
        FWD = cls(subdir, simname, machine = machine, **kwargs)
        inp    = FWD.input
        output = FWD.output
        ...
        if kwargs.get('welch_spectra', False):
            ...
            freqs, Sk = np.fft.fftshift(freqs), np.fft.fftshift(Sk)
            freqs *= 1e4
        else:
            Sk = np.fft.fftshift(np.abs(np.fft.fft(signal)))
            freqs = np.fft.fftshift(np.fft.fftfreq(signal.size, d = 1e-4))
        ...
        out = Struct()
        out.angle         = np.abs(inp.angle)
        out.k_perp        = 4 * PI * inp.f0 / C * np.sin(np.abs(inp.angle) * PI / 180) / 100  # cm^-1
        out.fD            = fD
        out.fD_Sim        = Freqs[np.argmax(Signal)] 
        out.fD_Lor        = Freqs[np.argmax(lor_fit)] 
       # out.fD_Gau        = Freqs[np.argmax(gau_fit)] 
        out.freqs         = freqs
        out.Freqs         = Freqs
        out.Raw_signal    = Sk
        out.Signal        = Signal
        out.lorentian_fit = lor_fit
        #out.gaussian_fit  = gau_fit
        #out.f_mean        = np.mean([out.fD_Lor, out.fD_Gau])
        #out.f_std         = np.std([out.fD_Sim, out.fD_Lor, out.fD_Gau])
        # out.f_err         = max(abs(out.f_mean - f) for f in [out.fD_Sim, out.fD_Lor, out.fD_Gau]),
        out.validated     = kwargs.get('validated', 0)
...
if __name__ == '__main__':
    subdir = 'FWS_advection'
    machine = 'irene'
    simname = 'advection_f60_angle15'
