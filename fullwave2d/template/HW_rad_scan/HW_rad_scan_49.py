...
mode            = 'O'
f0, nx, ny, dx  = 49e9, delta_ne.shape[0], delta_ne.shape[1], dx
b0              = 2.5
waist, yante    = 120 * dx, int(ny / 2 - 250) * dx
angle           = 10
t_step          = 1
nt              = int(8e3)
B0              = (b0 * np.ones([nx, ny])).astype(np.double)

subdir          = 'HW_rad_scan_2'
...
if not size==1:
    # print('in the mpi')
    t0 = time.time()
    outp_gathered = scatterv_maxwell_HW(inp, ne_lin, filename, t_start=10, t_end=None, root=0, fluct_lvl=0.3, simulations_per_CPU = simulations_per_CPU, t_step = t_step)
    # save the results
    if rank == root:
