...
def show_estimate_with_errorbars(ax, specobj, **kwargs):
    ...
    fDop_vline = ax.axvline(fDop/1e3, color=col, lw=lw, ls='--', **kwargs)
    
    dfDop_hline = plot_semi_log10(ax,np.array([fDop_min, fDop_max]) / 1e3, y_errorbar * np.array([1,1]), color=col, lw=lw, **kwargs)
...
# %%
    subdir = 'mixed_advection_2_large'
    dataI  = DataInterface(subdir)
    wrapper = OutputWrapper(dataI)
# %%
