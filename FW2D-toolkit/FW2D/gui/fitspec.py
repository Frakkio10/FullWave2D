...
class FitSpec(QtWidgets.QWidget):
    ...
    def _setup_ui_elements(self, N_plots, sharex=True, sharey=True):
        ...
        self.axs_amplitude   = [AxesWidget() for i in range(N_plots)]
        share_axes(self.axs_signal, axis='y')
        share_axes(self.axs_amplitude  , axis='y')
        
        for ax_sig, ax_amplitude in zip(self.axs_signal, self.axs_amplitude):
            
            ax_sig.hover_label.hide()
            # ax_amplitude.hover_label.hide()
            
            share_axes([ax_sig], axis='x')
            share_axes([ax_amplitude], axis='x')
            ...
            ax_sig.set_ylabel('')
            ax_amplitude.set_xlabel(r"10Δt")
            ax_amplitude.set_ylabel('') # will be assigned by the spectrogram display method
        ...
        for i, (data_panel, ax_sig, ax_amplitude, ax) in enumerate(zip(self.data_panels, self.axs_signal, self.axs_amplitude, self.axs)):
            container = QWidget()
        ...
        self.navigator = FitSpecWidgetNavigator(
            [self.axs, self.rhs_panel], type='list', descriptions=[f'{i}' for i in self.isims])
    ...
    def _setup_ui_interaction(self):
        ...
        for i, (s, ax, ax_sig, data_panel) in enumerate(zip(self.specobjs, np.array(self.axs).flatten(), self.axs_signal, self.data_panels )):
            ...
            make_title(ax, self.data_interface, s.header.isim)
    ...
    def _perform_fit(self, ax, specobj, data_panel, include_mask=None, **kwargs):
        ...
        tmp = include_mask
        perform_specobj_fits(specobj, include_mask = include_mask, p0 = p0, reinitialize = reinitialize, verbose = self.verbose)
        self._update_fit(ax, specobj, data_panel, reset_panel=True, reset_validation=True, **kwargs)
        if not np.allclose(tmp, specobj.include_mask):
            print('wrong')
    ...
    def _plot_amplitudes(self):
        ...
        for ax in self.axs_amplitude:
            t, ampl, phase = ax.t, ax.ampl, ax.phase
            ax.plot(t, ampl / ampl.max(), color = 'r', lw = 2, label = 'Amplitude [a.u.]')
            ax.plot(t, phase / np.pi / 2, color = 'b', lw = 2, label = 'Phase [a.u.]')
            ax.legend()
    ...
    def _update_signal_plot(self, ax_sig, specobj, roi=None):
        ...
        if not hasattr(ax, 'plot_dict'): # plot initializatio
            
            t,I,Q = np.copy(ax.t), np.copy(ax.I), np.copy(ax.Q)
            ...
            lQ = ax.plot(t, Q / np.pi, color='b', alpha=0.6, label='Q')
    ...
    def _on_refit_with_init_guess(self):
        ...
        p0 = [1.0, specobj.fDop / specobj.xscale, 0.5]
    ...
    def _load_amplitude(self):
        ...
        worker = WorkerAmplitude(self)
        worker.data_loaded.connect(self._plot_amplitudes)
        worker.finished.connect(self._cleanup_worker)
        self.workers.append(worker)
        worker.start()
...
class WorkerAmplitude(QThread):
    ...
    def run(self):        
        ...
        for (ax, isim) in zip(self.parent.axs_amplitude, self.parent.isims):
            t, ampl, phase = self.parent.data_interface.get_amplitude_phase(isim)
            ax.t = t; ax.ampl = ampl; ax.phase = phase;
