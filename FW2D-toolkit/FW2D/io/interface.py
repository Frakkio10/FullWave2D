...
class DataInterface(Struct):
    ...
    def get_amplitude_phase(self, isim = None):
        if isim is None:
            isim = input_prompt(f'Which simulation? from 1 to {self.params.Nbsim +1}')

        out = OutputData(self.params.name[isim -1], subdir=self.subdir, machine = self.machine)
        inp = InputData.load_pickle(self.params.name[isim -1], subdir = self.subdir, machine = self.machine)
        t = np.linspace(0, inp.nt / 10, int(inp.nt/ 10))
        ampl, phase = out.ampl, out.phase
