...
def preprocessing(z, dt, noise_freq_range=[0.5*4.9e3, 4.9e3], nperseg=512, noverlap=256, fmin=100, fmax=50, spikes_removal_threshold=3, **kwargs):
...
def init_specobjs(data_interface, isims, time_mask_xarray=None, verbose=False, nperseg=512, noverlap=256, **kwargs):
    ...
    for isim in isims:
        ...
        noise_freq_range = [0.5*4.9e3, 4.9e3]
        specobj = Struct()
...
def perform_specobj_fits(specobj, include_mask=None, p0=None, reinitialize=False, signal_include_mask=None, verbose=False):
    ...
    if include_mask is None:
        include_mask = get_frequency_range(s.f, fmin=50, fmax=50)
    ...
    #     updated_specobj = init_specobjs(dataI, ifreqs=[specobj.header.isim], signal_include_mask=signal_include_mask)[0]
...
def get_fDop_from_fit_results(fit_params, xscale=1e3, f_cog=None):
...
def make_title(ax, data_interface, isim):
    par = data_interface.params
    freq, theta, waist, mode = par.F[isim], par.theta[isim], par.waist[isim], par.mode[isim]

    title = r'f = %.1f GHz, $\theta$ = %d, waist = %d, %s mode' %(freq, theta, waist, mode)
    ax.set_title(title)
...
def show_spec(specobj, ax=None, axis_labels=True, 
    ...
    for show_c in show_curves:
        ...
        if show_c == 'rawPSD':
            ...
            lrawPSD = plot_semi_log10(ax, _xraw / xscale, _yraw + s.P_noise, label=lab, alpha=0.7, **plot_kwargs)
            plot_dict['lrawPSD'] = lrawPSD
            
        elif show_c == 'fittedPSD':
            lab = plot_kwargs.pop('label', 'fitted PSD')
            lfittedPSD = plot_semi_log10(ax, xdata , ydata * yscale + s.P_noise, color='red', label=lab, **plot_kwargs)
    ...
    if not hasattr(s, 'fit_params'):
        ...
        if 'annotations' in show_curves:
            ...
            cog_vline = ax.axvline(s.f_cog/xscale, color=colors['odd'], ls='-.', label='odd CoG')
...
def show_estimate_with_errorbars(ax, specobj, **kwargs):
    ...
    fDop_vline = ax.axvline(fDop/1e6, color=col, lw=lw, ls='--', **kwargs)
    
    dfDop_hline = plot_semi_log10(ax,np.array([fDop_min, fDop_max]) / 1e6, y_errorbar * np.array([1,1]), color=col, lw=lw, **kwargs)
...
class OutputWrapper():
    ...
    def _get_outpath(self)->Path:
        ...
        outpath = defs.DATA_FDOP_DIR / f'{dataI.subdir}.mat'
        return outpath
    ...
    def _make_toplevel_data(self, merged_specobj):
        ...
        # update also header:
        merged_specobj.header.ifreqs_treated = np.array(indices_treated)
# %%
if __name__ == '__main__':
    ...
    isims = dataI._get_sim_choice([])
    specobjs = init_specobjs(dataI, isims=isims)
    
    for s in specobjs[:2]:
        perform_specobj_fits(s)
    
    wrapper.update_specobjs(specobjs)

    show_spec(specobjs[0])
# %%
